...
<form class="compte-form-container" action="${pageContext.request.contextPath}/crudCompte?action=update" method="post">
    <h1>Modifier un compte</h1>
    <input type="hidden" name="compteId" value="${compte.id}">
    <input type="text" name="userId" value="${compte.user.getId()}">
    
    <select name="type_compte" required>
        <option value="Epargne" ${compte.type_compte == "Epargne" ? 'selected' : '' }>Compte d'Épargne</option>
        <option value="Courant" ${compte.type_compte == "Courant" ? 'selected' : '' }>Compte Courant</option>
    </select>
  
    <input type="text" name="numCompte" value= "${compte.numCompte}" required>
    <input type="text" name="solde" value = "${compte.solde}" required>
